    ...
    run:async(client, message, args)=>{
        ...
        if(!args[0])return message.reply('Role Was Not Specified')
            ...
            if(message.mentions.roles.first()){
                ...
                embed.setTitle('Role Information for '+`${role.name}`)
                .addField('Name', role.name, true)
                .addField('ID', role.id.toString(), true)
                .addField('Created At', role.createdAt.toString(), true,)
                .addField('Color', role.color.toString(), true, )
                .addField('Display Seperate', role.hoist?'Yes':'No', true, )
                .addField('Members Who Have This Role', role.members.size.toString(), true,)
                ...
                const role=message.guild.roles.cache.find(r=>r.name.toLowerCase()===roleToBeFound)
                console.log(role)
