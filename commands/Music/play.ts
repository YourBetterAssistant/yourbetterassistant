...
module.exports = {
  ...
  run: async (client: Client, message: Message, args: string[]) => {
    ...
    if (message.member.voice.channel.id !== message.guild.me.voice.channel.id)
      return message.channel.send(
        "You are not in the same voice channel as me, join this VC: " +
          `<#${message.guild.me.voice.channel.id}>`
      );
    if (!args[0])
      return message.channel.send("What song do you want me to do?");
    const results = await client.lavalink.rest.loadTracks(
      `ytsearch:${args.join(" ")}`
    );
    ...
    server_queue.player.queue.add([results.tracks[0]], {
    ...
    if (server_queue.player.playing) {
      message.channel.send(
        `Added to queue:  \`${results.tracks[0].info.title}\` `
      );
    }
