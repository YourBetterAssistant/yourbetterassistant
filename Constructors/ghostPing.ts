...
export const messageUpdate = async (oldMessage: any, newMessage: any) => {
  ...
  if (oldMessage.mentions.users.size >= 1) {
    olduserMentioned = true;
  }
  if (oldMessage.mentions.roles.size >= 1) {
    oldroleMentioned = true;
  }
  if (oldMessage.mentions.everyone.size >= 1) {
    oldeveryoneMentioned = true;
  }
  if (oldMessage.mentions.members.size >= 1) {
    oldmemberMentioned = true;
  }
  //New Message
  ...
  if (newMessage.mentions.users.size >= 1) {
    newuserMentioned = true;
  }
  if (newMessage.mentions.roles.size >= 1) {
    newroleMentioned = true;
  }
  if (newMessage.mentions.everyone.size >= 1) {
    neweveryoneMentioned = true;
  }
  if (newMessage.mentions.members.size >= 1) {
    newmemberMentioned = true;
  }
  ...
  if (
    oldroleMentioned === false &&
    olduserMentioned === false &&
    oldmemberMentioned === false &&
    oldeveryoneMentioned === false &&
    newroleMentioned === false &&
    newuserMentioned === false &&
    newmemberMentioned === false &&
    neweveryoneMentioned === false
  )
    return;
  if (
    oldroleMentioned === false &&
  ...
  const embed = new MessageEmbed().setTitle("Ghost Ping!");
  let i = 1;
  ...
  embed.setAuthor(
    ...
    newMessage.author.displayAvatarURL("png", { dynamic: true })
  );
...
export const messageDelete = async (message: any) => {
  ...
  if (message.mentions.everyone.size >= 1) {
  ...
  embed.setAuthor(
    `${message.author.tag}`,
    message.author.displayAvatarURL("png", { dynamic: true })
  );
  embed.setColor("GREEN").setTimestamp(new Date());
